...
class LocationsQueries(BaseQueries):
    def insert_location(self, barcode: str, type_id: int, group_id: int) -> dict:
        query = "INSERT INTO locations VALUES (DEFAULT, %s, %s, %s) RETURNING id"
        return self.execute(query, (barcode, type_id, group_id))[0]

    def select_location(self, location_id: int) -> dict:
        query = "SELECT id, barcode, type_id, group_id FROM locations WHERE id = %s"
